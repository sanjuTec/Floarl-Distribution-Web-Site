<?php 
	...
	$msg="";
		if(isset($_POST['submit'])){
			...
			$newpwd = md5($pwd);
			$sql = "SELECT * FROM admin WHERE email='$email' AND password='$newpwd'";
			if($qu=mysqli_query($conn,$sql)){
				while($row= mysqli_fetch_array($qu)){ 
					$_SESSION['current_user']=$row['fname'];
...
body{
	background-image:url(img/bg-img/profile2.jpg);
 ...
 <img src="img/login02_popup.png" alt="">
  ...
  <div class="ebook-email-sec">
	<form action="login.php" method="post">
		<input type="email" name="email" class="ebookemail-input1" data-edit="placeholder" placeholder="Enter Email Address" required>
		<input type="password" name="password" class="ebookemail-input2" data-edit="placeholder" placeholder="Enter Password" required>
		<div>
			<?php
				echo "<font color='red'>";
					echo $msg;
		...
		<input name="submit" class="ebook-input-btn" type="submit" value="logIn">
    </form>
  </div>
  <h5>Not Registered? <span><a href="join.php">Click Here</a></span> to Register</h5>
</div>    
</section>
</body>
</html>
